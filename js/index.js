...
async function GetTimezone (CityName) {
    try  {
        ...
        var respone = await fetch(`https://api.weatherapi.com/v1/timezone.json?key=192cae221c2840bc959134852240612&q=${CityName}`+ Date.now());
...
async function Getweather (CityName="cairo") {
 ...
 var afterTomorrow = GetdayAfterTomorrow_DATE(cureentDate) 
        ...
        var respone = await Promise.all([
            fetch(`https://api.weatherapi.com/v1/forecast.json?key=192cae221c2840bc959134852240612&q=${CityName}&dt=${tomorrow.year}-${tomorrow.month}-${tomorrow.day}`+ Date.now()),
             fetch(`https://api.weatherapi.com/v1/forecast.json?key=192cae221c2840bc959134852240612&q=${CityName}&dt=${afterTomorrow.year}-${afterTomorrow.month}-${afterTomorrow.day}`+ Date.now())
...
document.getElementById("weatherbox").classList.replace("invisible","visible") 
 ...
 async function  getWEATHERFORcurrentLOCATON(){
    ...
    try{  
        ...
        await Getweather (cityCoordinates)
    } 
    catch(error){
